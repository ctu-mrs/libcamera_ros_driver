...
def generate_launch_description():
    ...
    declare_standalone = DeclareLaunchArgument(
        "standalone", default_value="true", description="Whether to start a as a standalone or load into an existing container."
    )
    ...
    declare_container_name = DeclareLaunchArgument(
        "container_name", default_value="", description="Name of an existing container to load into (if standalone is false)"
    )
    ...
    ld.add_action(
        SetEnvironmentVariable(
            name="LIBPISP_BE_CONFIG_FILE", value=os.environ.get("LIBPISP_BE_CONFIG_FILE", f"{libcamera_path}/share/libpisp/backend_default_config.json")
        )
    )

    ld.add_action(
        SetEnvironmentVariable(name="LIBCAMERA_IPA_MODULE_PATH", value=os.environ.get("LIBCAMERA_IPA_MODULE_PATH", f"{libcamera_path}/lib/libcamera/ipa/"))
    )

    ld.add_action(
        SetEnvironmentVariable(name="LIBCAMERA_IPA_CONFIG_PATH", value=os.environ.get("LIBCAMERA_IPA_CONFIG_PATH", f"{libcamera_path}/share/libcamera/ipa"))
    )
    ...
    ld.add_action(
        DeclareLaunchArgument(
            "camera_name", default_value="front", description="Name of the camera (used to select which camera to use if multiple are present)"
        )
    )
    ...
    load_into_existing = LoadComposableNodes(
        target_container=container_name, composable_node_descriptions=[default_node], condition=UnlessCondition(standalone)
    )
    ...
    standalone_container = ComposableNodeContainer(
        ...
        parameters=[{"thread_num": os.cpu_count()}, {"use_sim_time": use_sim_time}],
