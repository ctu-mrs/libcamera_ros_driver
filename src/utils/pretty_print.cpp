...
std::ostream& operator<<(std::ostream& out, const libcamera::CameraManager& camera_manager)
{
  ...
  for (size_t id = 0; id < camera_manager.cameras().size(); id++)
  {
    ...
    const std::string name = camera->properties().get(libcamera::properties::Model).value_or("UNDEFINED");
    out << std::endl << "   " << id << ": " << name << " (" << camera->id() << ")";
  }
  return out;
}

std::ostream& operator<<(std::ostream& out, const libcamera::StreamFormats& formats)
{
  ...
  {
    out << std::endl
        << "   - Pixelformat: " << pixelformat.toString() << " (" << formats.range(pixelformat).min.toString() << " - "
        << formats.range(pixelformat).max.toString() << ")";
  }
  return out;
}

std::ostream& operator<<(std::ostream& out, const libcamera::StreamConfiguration& configuration)
{
